...
### 修复
- 实现已发布 `Unpaid` invoice gateway switch 候选路径：在 production-canary guard 通过后使用 WHMCS `UpdateInvoice` Local API add/remove 本模块 fee line item；`stripe` / `stripealipay` 之间切换只更新审计 gateway，不重复新增 fee。
- 修复 production canary P0 安全问题：默认 `enabled=off`，canary disabled 不再放行写入，automation 批量扫描始终阻断；只有显式启用、关闭 dry-run、kill switch 关闭且 invoice/client allowlist 精确匹配时才允许写入。
...
- 明确 WHMCS 9.0 immutable invoice 风险边界：已发布 invoice 的 gateway 切换现在只在 production-canary guard 通过后走受控 `UpdateInvoice` add/remove。
...
- 后台页面显示配置、最近 50 条 fee 审计记录，并提供 dry-run / canary-safe invoice check 工具。
